...
function data_frame(data_raw)
    ...
    disp('------------------------------------>Successfully run : IEEE_C37_118 -- data_frame<-------------'); 
    ...
    if SADF.tcp
        ...
        if (SADF.verbose_info || SADF.verbose_debug)
            disp([datestr(now, 'dd/mm/yyyy HH:MM:SS.FFF') '    RCV - IEEE C37.118.2 DATA']);
        end
        ...
        elseif index_tmp > DATA.index_max + 1
            ...
            process_frame = true;
            if (SADF.verbose_info || SADF.verbose_debug)
                disp([datestr(now, 'dd/mm/yyyy HH:MM:SS.FFF') '    RCV - IEEE C37.118.2 DATA, out-of-order and/or missing']);
            end
        ...
        elseif index_tmp < DATA.index_max+1 && ~isnan(DATA.TimeStamp(index_tmp,1))
            ...
            process_frame = false;
            if (SADF.verbose_info || SADF.verbose_debug)
                disp([datestr(now, 'dd/mm/yyyy HH:MM:SS.FFF') '    RCV - IEEE C37.118.2 DATA, duplicated']);
            end
        end
    end
    ...
    if process_frame
        ...
        if (SADF.verbose_debug)
            ...
            disp(['  Timestamp: ' datestr(SOC + (FOS / CFG_2_3.Time_Base(CFG_2_3.counter, 1) / 86400), 'dd/mm/yyyy HH:MM:SS.FFF')]);
        ...
        for pmui = 1 : CFG_2_3.Num_PMU(CFG_2_3.counter, 1)
           for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
               if pmui == 1
                ...
                fprintf('------------------------------------>Successfully run :  127 data_frame -- loop start pmui= %d   phasori = %d<-------------',pmui,phasori);
            %status flags
            a1 = a2 + 1; 
            a2 = a2 + 2;
            ...
            if CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter) > 0
                ...
                if SADF.conf_fr_ver == 2 % Config 2
                    if CFG_2_3.PMUsDataFormat(8, pmui, CFG_2_3.counter)==1   %Phasor magnitude and angle (polar)
                        if CFG_2_3.PMUsDataFormat(7, pmui, CFG_2_3.counter)==1   %IEEE floating point , NO SCALING NEEDED
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                ...
                                a2 = a2 + 4;
                                %Magnitude
                                fprintf('------------------------------------>Successfully run :239 data_frame  if 231 -- condn 1 236 with phasori=%d<-------------',phasori); 
                            ...
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                ...
                                a2 = a2 + 2;
                                %Magnitude
                                fprintf('------------------------------------>Successfully run : data_frame if -- condn 2 249 with phasori= %d<-------------',phasori)
                        ...
                        if CFG_2_3.PMUsDataFormat(7, pmui, CFG_2_3.counter)==1 %IEEE floating point
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                a1 = a2 + 1;
                                a2 = a2 + 4;
                                 fprintf('------------------------------------>Successfully run : data_frame else 260 -- condn 2 265 with %d<-------------',phasori);
                                %Real
                            ...
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                ...
                                a2 = a2 + 2;
                                %Real
                                fprintf('------------------------------------>Successfully run :284 data_frame -- else condn 2 284 with phasori = %d<-------------',phasori);
                    ...
                    if CFG_2_3.PMUsDataFormat(8, pmui, CFG_2_3.counter)==1 %Phasor magnitude and angle (polar)
                        if CFG_2_3.PMUsDataFormat(7, pmui, CFG_2_3.counter)==1 %IEEE floating point
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                fprintf('------------------------------------>Successfully run : data_frame -- cond 3 305 with phasori = %d<-------------',phasori);
                                a1 = a2 + 1;
                                a2 = a2 + 4;
                                %Magnitude
                                DATA.Magnitude(DATA.index, phasori, pmui) = swapbytes(typecast(swapbytes(data_raw(a1:a2)),'single'));
                            ...
                            for phasori = 1 : CFG_2_3.PMUsPhasorNum(1, pmui, CFG_2_3.counter)
                                a1 = a2 + 1;
                                a2 = a2 + 2;
                                 fprintf('------------------------------------>Successfully run : data_frame -- 16 bit cond 3 305 318 with %d<-------------',phasori);
                               %Magnitude
